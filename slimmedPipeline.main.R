...
setwd("G:/450K_DataProcessing/")
#
# If working on Windows set this:
memory.limit(200000)
#
# set PATH to results folder
PATH_RES <- "G:/450K_DataProcessing/"
#
...
projectName = "ILLUMINA450K_AllBlood_p2"
#
...
#listOfinterstSamples <- read.delim("G:/450K_DataProcessing/CombinedSampleSheet.txt", as.is=T)
#unMeth <- unMeth[,-which(colnames(unMeth)%in%listOfinterstSamples$Methylation.ID[which(listOfinterstSamples$Tissue!="Blood")])]
#meth <- meth[,-which(colnames(meth)%in%listOfinterstSamples$Methylation.ID[which(listOfinterstSamples$Tissue!="Blood")])]

## You can check some QC here potentially(?)
